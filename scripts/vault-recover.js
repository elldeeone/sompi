 ...
 *     <ownerPrivateKey> <agentPublicKey> <maxOutflowSompi> <destinationAddress> [feeSompi]
 */
...
async function main() {
  const [ownerPriv, agentPublic, maxOutflow, destination, fee] = process.argv.slice(2);
  if (!ownerPriv || !agentPublic || !maxOutflow || !destination) {
    console.error("usage: vault-recover.js <ownerPrivateKey> <agentPublicKey> <maxOutflowSompi> <destination> [feeSompi]");
    process.exit(2);
  }
  ...
  const { VaultManager } = require("../dist/vault");
  ...
  const redeem = buildRedeemScript(agentPublic, ownerPublic, BigInt(maxOutflow));
