...
|---|---|---|
| Start a purchase | Stable `requestKey`, resource URL, method, optional body | Review Authority terminal when prompted |
| Check progress | Existing Purchase ID | None |
| Recover interruption | Existing Purchase ID | Follow the projected operator action |
| Approve | None through API or MCP | Type the exact Purchase ID in `sompi-authority` |
...
- `awaiting_authority`: review the exact facts in the trusted Authority
  terminal.
